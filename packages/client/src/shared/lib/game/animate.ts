...
type ObserverValuesType = (() => void)[];

class Observer {
  obj: Record<ObserverTypes, ObserverValuesType> = {
    [ObserverTypes.create]: [],
    [ObserverTypes.update]: [],
    [ObserverTypes.finish]: [],
  };
  ...
  remove(o: ObserverTypes, cb: () => void) {
    this.obj[o] = this.obj[o].filter((cbFromObj) => cbFromObj === cb);
  }
}
